...
> 带 prefers-reduced-motion 降级，常量驱动并暴露在控制台面）**，
> **rev-18 按真机反馈把那个转动放慢（160ms → 300ms），并新增顶层诊断 `reduceMotion()`，
> 让「时长太短」与「环境要求减少动效」这两种看起来一样的原因可被区分**。
> 每次改动后都复跑了全部 harness：现为 4 个文件 / **620 项断言全绿（124 + 399 + 22 + 75）**，
> 另有**独立验证层** 6 个探针 / **402 项**，加**重基线后全绿的 10 个遗留探针 / 820 项**（`verify-independent/`，见下条）。
