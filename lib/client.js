...
(function () {
  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
       ...
       * behavioural edit.
       */
      var REVISION = 'rev-24 · muting draws the slash instead of moving the bell';
      /**
      ...
      /**
       * The STRIKE-THROUGH (rev-24, user request: "不要晃动，静音时把斜杠重左上拉到右下的动画" — the bell
       * must not move at all, and the MUTE is what should animate).
       *
       * So the bell body no longer animates in ANY state: rev-23's tilt and rev-22's rattle are both gone,
       * and the paragraph that follows is kept as the record of what was tried and rejected. A bell that
       * wobbles draws the eye to the wrong thing — the action the user performs is MUTE, so the mute is
       * what gets drawn: on going muted, the slash's STROKE is drawn from its top-left end to its
       * bottom-right end. Nothing is translated, rotated or scaled.
       *
       * THE TWO REJECTED MOTIONS, kept as the record of what this replaced. Both were motion OF THE BELL
       * BODY, which is precisely what the current request forbids:
       *   - rev-22 drew a damped RING: four diminishing swings (0 → +14° → −11.06° → +7° → −4.06° → 0)
       *     over 420 ms on `ease-in-out`. Read as a shake;
       *   - rev-23 replaced it with ONE lean past rest and a settle (9° peak, 560 ms, a long-tail ease-out
       *     plus a small scale track), on the argument that "premium" for a motion this small is a long
       *     arrival rather than a rattle. The user rejected the whole family: the bell must not move at
       *     all, and the MUTE is what should animate.
       * Neither version's constants survive, and their FORMER NAMES are deliberately not quoted here: a
       * dead identifier in a comment is how a later reader concludes the code still owns it. The guard in
       * client-half greps for exactly that mistake.
       *
       * WHAT moves: the slash's stroke, and only it. The glyph and the 28px button are untransformed in
       * every state — the same rule the caret's turn follows, for the same reason (the button IS the hover
       * pill; animating that box would move the pointer's own background).
       *
       * WHEN it moves: on the click that flips this session — NOT on mount and NOT on hover. That is the
       * same shape the caret follows: it animates on the state it reports, not on attention. The slash's
       * arrival IS the acknowledgement of the mute the user just performed, and the paint flips blue↔grey
       * at that same moment. A slash that drew itself on page load would claim an event that never happened.
       *
       * HOW it replays: the glyph is remounted by a React `key` carrying a counter that the click handler
       * bumps, and the stylesheet arms the animation only once that counter is non-zero. A CSS animation
       * restarts when its element is replaced, so N toggles are N draws; gating on the counter is what
       * keeps the FIRST paint silent.
       *
       * ONE duration and TWO curves drive all four halves of the gesture. Both are declared at
       * BELL_MUTE_MS below, together with the tuning history that produced them. Nothing here is derived
       * from the caret's 160 ms and nothing there from this: do not copy one number for the other's job,
       * and if a device shows a jump rather than a draw, that constant is the number to move.
       *
       * No `prefers-reduced-motion` override is added for the slash, matching rev-20's decision for the
       * caret (see the note above CARET_OPEN_ROTATE_DEG): one rule, every environment, with the trade-off
       * written down instead of dressed up. Do not re-add one here.
       */
      /** The slash's two ends, in viewBox units: top-left and bottom-right, on both axes. */
      var BELL_SLASH_FROM = 2.6;
      var BELL_SLASH_TO = 13.4;
      /**
       * The drawn length of that diagonal, DERIVED rather than transcribed: the path is a 45° line, so
       * its length is (TO − FROM)·√2. `stroke-dasharray` / `stroke-dashoffset` need exactly this number
       * to draw the stroke over time, and the path data below is built from the same two constants — so
       * the geometry, the dash length and the generated @keyframes text cannot drift apart.
       */
      var BELL_SLASH_LEN = Math.round((BELL_SLASH_TO - BELL_SLASH_FROM) * Math.SQRT2 * 100) / 100;
      /** The slash's stroke width, in viewBox units; the round cap extends half of it past each dash end. */
      var BELL_SLASH_STROKE = 1.7;
      /**
       * WHY THE INVISIBLE STATE IS `opacity`, NOT A DASH OFFSET — two failed attempts, both seen by the user.
       *
       * The slash has to exist while the bell is AUDIBLE (the un-mute animation needs an element to play on)
       * and it has to draw nothing there. Hiding it with `stroke-dashoffset` was tried twice and failed twice,
       * because `stroke-dasharray` with a single value REPEATS every 2·LEN:
       *   - `dashoffset: LEN` — and its negative twin `−LEN` — parks a dash boundary exactly on a path END, and
       *     `stroke-linecap: round` paints a semicircle of radius strokeWidth/2 there: a DOT. The user saw the
       *     bottom-right one after un-muting ("为什么右下角有个点");
       *   - pushing the dash one length PAST the end (`LEN + strokeWidth/2 + margin`) does clear that end — and
       *     pulls the NEXT repetition of the dash in through the other one, leaving a short LINE at the
       *     top-left end. The user saw that too ("现在是左上角有个线").
       * A repeating pattern cannot be moved clear of BOTH ends of a path whose length IS the dash length. So the
       * resting state is hidden by `opacity`, which no dash geometry can defeat, and both keyframes animate
       * `opacity` alongside `stroke-dashoffset` — which also makes each animation FINISH on a state that is
       * genuinely invisible, rather than on one that merely happens to put no ink on the path.
       */
      /**
       * How long ONE mute gesture takes. It covers BOTH things that move — the slash's stroke and the blue fill
       * — and both directions (user request: "关闭静音的时候斜杠从左上到右下动画，两个动画时长一样"). Turning the
       * mute ON draws the slash and dims the fill; turning it OFF sweeps the slash away and brings the fill back.
       * One constant, so the four halves of the gesture cannot drift apart in timing.
       *
       * THE TUNING HISTORY, because four rounds of reader feedback ARE the specification here:
       *   240 ms — the first cut; reported too fast ("斜杠有点快了慢点"). Note WHAT that 240 covered: the
       *     stroke alone. The blue fill did not animate yet, so that verdict was passed on a SMALLER
       *     gesture than the one this constant now drives — see the last entry;
       *   420 ms — the stroke alone, and the way OUT was reported slow ("又有点慢了");
       *   420 ms + the blue fill — reported too slow, and THIS is the informative data point: the same
       *     duration now covers more visual change (a large area dimming as well as a stroke travelling),
       *     so the gesture reads slower than the stroke did alone at 420;
       *   300 ms — the two-part gesture at 300, still reported slow ("快点");
       *   360 ms — held for ONE round because a typo was read literally: the reader wrote 「在慢点」 meaning
       *     「再快点」 ("是快点打错字了"). 360 was therefore never judged on its merits and must NOT be
       *     cited later as a value the reader rejected — it was never really on trial;
       *   240 ms — the value below, and a NEW measurement rather than a retry: the fill moves with the
       *     stroke now, so 240 covers more visual change than the 240 that was called fast did, i.e. it
       *     reads slower than that one. That is the whole argument for returning to a rejected number.
       *     If it lands fast again, the midpoint of the two rejected ends (270) is the next value — not a
       *     fresh guess, and not another slide along the same axis.
       * This is the ONE knob: the two easing curves, the slash's geometry and the fill's midpoint are all
       * derived from it or declared beside it, so moving this number moves the whole gesture and nothing else.
       */
      var BELL_MUTE_MS = 240;
      /** The draw: leaves quickly and spends the rest arriving — a pen decelerating as it lands. */
      var BELL_SLASH_DRAW_EASE = 'cubic-bezier(0.22,1,0.36,1)';
      /**
       * The sweep's curve: `cubic-bezier(0.42,0,0.58,1)` — the plain `ease-in-out`, chosen after THREE rounds
       * of reader feedback, and deliberately NOT the exact time-reverse of the draw's curve.
       *
       * The history is the argument, so it is kept: attempt 1 used the DRAW'S OWN ease-out, and the ink
       * vanished early ("斜杠太快了"); attempt 2 used that curve's exact mirror, `cubic-bezier(0.64,0,0.78,0)`
       * — mathematically symmetric, but a strong ease-in spends its first ~40% barely moving, which reads as a
       * dead pause rather than as a slower stroke ("又有点慢了"). Both complaints are about the same thing seen
       * from opposite sides: an ease curve with a long plateau at one end. `ease-in-out` has neither a dead
       * lead-in nor an early exit, so the whole 420 ms is motion.
       *
       * It is also worth recording that this curve is its OWN time-reverse — `(1−x2, 1−y2, 1−x1, 1−y1)`
       * returns the same four numbers — so the two animations remain symmetric in shape even though the two
       * constants are no longer each other's mirror.
       */
      var BELL_SLASH_SWEEP_EASE = 'cubic-bezier(0.42,0,0.58,1)';

      /**
       * Render every keyframe of the MUTE GESTURE from the constants above: the slash's draw and sweep, and the
       * blue fill's dim and glow.
       *
       * A function rather than a literal string so the dash travel IS the diagonal's own length and the
       * duration IS the one shared constant: the emitted stylesheet cannot disagree with the console surface
       * about the geometry, and the two directions cannot drift apart in timing. (This comment used to describe
       * the rev-23 tilt and was left behind when the motion was replaced — recorded rather than quietly
       * deleted, because a stale comment is exactly how the next reader learns something false about the code.)
       */
      function bellMuteKeyframes() {
        return '@keyframes dacSlashDraw{from{stroke-dashoffset:' + String(BELL_SLASH_LEN) + ';opacity:0;}'
          + 'to{stroke-dashoffset:0;opacity:1;}}'
          // The inverse of the draw, from the SAME numbers. A NEGATIVE offset pushes the dash the other way,
          // so the ink that remains is the tail end of the path: the stroke is consumed starting at its
          // top-left end and the ink retreats towards the bottom-right one — the same direction the draw
          // travels, which is what makes the pair read as one gesture played forwards and backwards.
          //
          // Both keyframes end on `opacity`, and that is load-bearing rather than decorative: the sweep runs
          // while the bell is AUDIBLE (where the resting rule says `opacity: 0`), and `forwards` keeps only the
          // ANIMATED properties — so without an animated opacity the finished sweep would fall back to the
          // resting rule with the dash parked on the end point, i.e. the bottom-right dot again.
          + '@keyframes dacSlashSweep{from{stroke-dashoffset:0;opacity:1;}'
          + 'to{stroke-dashoffset:-' + String(BELL_SLASH_LEN) + ';opacity:0;}}'
          // The FILL, on the same clock (user request: "蓝色的部分也弄个逐渐变暗到消失的动画"). `dim` runs while
          // the bell is muted and `glow` while it is audible, so the pair is each other's time-reverse. The
          // midpoint is BELL_ON_BG_HOVER — the darkened blue this file ALREADY derives for the hover state —
          // reused here as the "gradually darkening" step, so the dim has a DEFINED shape instead of relying on
          // the fact that CSS happens to interpolate towards `transparent` through darker and darker blues.
          + '@keyframes dacBellDim{0%{background-color:' + BELL_ON_BG + ';}'
          + '55%{background-color:' + BELL_ON_BG_HOVER + ';}'
          + '100%{background-color:transparent;}}'
          + '@keyframes dacBellGlow{0%{background-color:transparent;}'
          + '55%{background-color:' + BELL_ON_BG_HOVER + ';}'
          + '100%{background-color:' + BELL_ON_BG + ';}}';
      }
      ...
      function injectStyles() {
        ...
        try {
          ...
          style.textContent = [
            ...
            // rev-24 · the bell body NO LONGER MOVES (user request: "不要晃动"). rev-23's tilt rules are
            // DELETED outright: no `transform-origin` on the glyph, no animation on the glyph, no animation
            // on the button. client-half now asserts that ABSENCE rather than the old motion, because "the
            // bell does not move" is exactly the kind of property a later edit breaks by re-adding a
            // keyframe nobody asked for.
            //
            // What moves is the slash, and only its STROKE: the path is drawn from its top-left end to its
            // bottom-right end by animating `stroke-dashoffset` from the diagonal's own length to 0. Both
            // numbers come from the constants above and the @keyframes text is generated from them, so the
            // stylesheet cannot disagree with the console surface about the geometry it is drawing.
            //
            // `[data-draw="true"]` is what keeps the first paint still: the attribute is `false` until the
            // user has toggled this bell once, so a page that LOADS with a muted session shows the slash
            // already drawn instead of drawing it at the reader. After that each toggle replaces the glyph
            // (a new React key), and a freshly mounted element plays its animation from the first frame.
            '.dacSlash{stroke-dasharray:' + String(BELL_SLASH_LEN) + ';stroke-dashoffset:0;opacity:0;}',
            // The RESTING states are decoration-free by construction: the base rule draws the slash fully AND
            // hides it with `opacity`, so an audible bell can carry the node (which the sweep needs) without
            // showing any ink — no dot, no stub line, no dash geometry to get wrong. The muted state lifts the
            // opacity, and that is the whole of "drawn".
            '.dacBell[data-muted="true"] .dacSlash{opacity:1;}',
            // Two animations, ONE duration: both are spelled from BELL_MUTE_MS, so "两个动画时长一样" holds
            // by construction. `forwards` is what leaves the sweep's end state on the node — without it the
            // slash would snap back to fully drawn the instant the animation finished.
            '.dacBell[data-draw="true"][data-muted="true"] .dacSlash{animation:dacSlashDraw ' + String(BELL_MUTE_MS) + 'ms ' + BELL_SLASH_DRAW_EASE + ';}',
            '.dacBell[data-draw="true"][data-muted="false"] .dacSlash{animation:dacSlashSweep ' + String(BELL_MUTE_MS) + 'ms ' + BELL_SLASH_SWEEP_EASE + ' forwards;}',
            // The blue fill, on the button rather than on the glyph, and on the SAME clock as the stroke. No
            // `forwards` in either direction: the animation ENDS on the value the resting rule already has
            // (`transparent` when muted, BELL_ON_BG when audible), so the fill does not need to be held — and a
            // filled state would out-rank the hover rule for as long as it lasted.
            '.dacBell[data-draw="true"][data-muted="true"]{animation:dacBellDim ' + String(BELL_MUTE_MS) + 'ms ' + BELL_SLASH_SWEEP_EASE + ';}',
            '.dacBell[data-draw="true"][data-muted="false"]{animation:dacBellGlow ' + String(BELL_MUTE_MS) + 'ms ' + BELL_SLASH_DRAW_EASE + ';}',
            bellMuteKeyframes(),
       ...
       */
      function bellIcon(muted, drawCount) {
        // rev-24b · PAINT ORDER IS THIS ARRAY. SVG has no z-index: a later sibling paints on top of an earlier
        // one, so the slash used to be pushed LAST and therefore sat ABOVE the bell (user report: "斜杠的图层是
        // 在铃铛上面的"). The bell and its clapper are now pushed at the END of this function, which puts the
        // stroke BEHIND the bell's silhouette -- the intended reading. It also gives the draw a property worth
        // keeping: the ink emerges on the near side of the bell, vanishes behind it, and reappears on the far
        // side, so the animation reads as a stroke passing behind the icon rather than over it.
        var paths = [];
        // rev-24 · the node is kept once this bell has been toggled (`drawCount > 0`) even while AUDIBLE, so
        // that turning the mute OFF has an element to animate. There it draws nothing: the base rule draws the
        // dash in full and hides it with `opacity`, and the sweep rule plays that opacity down to zero. BEFORE
        // the first toggle the audible state still renders exactly two paths — which is what the rev-10 icon
        // contract and probe-18's recorded `{pathCount:2,hasSlash:false}` describe, and why that record holds.
        if (muted === true || (typeof drawCount === 'number' && drawCount > 0)) {
          paths.push(
            React.createElement('path', {
              key: 'slash',
              className: 'dacSlash',
              // rev-24 · built from the two endpoint constants, so the dash length the stylesheet animates
              // is the length of THIS line by construction rather than by a number copied next to it.
              d: 'M' + String(BELL_SLASH_FROM) + ' ' + String(BELL_SLASH_FROM)
                + ' ' + String(BELL_SLASH_TO) + ' ' + String(BELL_SLASH_TO),
              fill: 'none',
              stroke: 'currentColor',
              // Spelled from the constant so the stroke's own cap radius is the same number the geometry is
              // drawn with, instead of two 1.7s that agree until one of them is edited.
              strokeWidth: String(BELL_SLASH_STROKE),
              strokeLinecap: 'round',
            }),
          );
        }
        // rev-24b · pushed AFTER the slash on purpose: later siblings paint on top, so THIS is the rule that
        // puts the bell over the stroke. Keep the slash's push above this one -- moving it back down here would
        // silently restore the defect the user reported.
        paths.push(
          React.createElement('path', {
            key: 'bell',
        ...
        return React.createElement(
          'svg',
          {
            // rev-24 · the toggle counter IS this key: a changed key replaces the node, and a freshly
            // mounted slash plays its draw animation. Optional because the glyph is also drawn where
            // nothing toggles it — an absent counter means "never clicked".
            key: 'glyph' + String(typeof drawCount === 'number' ? drawCount : 0),
            viewBox: '0 0 16 16',
            width: BELL_GLYPH_PX,
      ...
      function SessionChimeAction(props) {
        ...
        /**
         * rev-24 · how many times THIS bell has been toggled. It does double duty: it is the React key of
         * the glyph (a new key replaces the <svg>, and a replaced element plays its animation from the
         * first frame), and it is what arms the slash draw at all — 0 means "never clicked", so a session
         * that is already muted when the page loads does not draw anything at the reader. Kept as one
         * counter rather than a boolean plus a separate key so the two can never disagree about whether a
         * draw is owed.
         */
        var drawState = React.useState(0);
        var drawCount = drawState[0];
        var setDrawCount = drawState[1];
        ...
        function toggle() {
          // rev-24 · bump BEFORE the write, so the draw starts on the same frame as the slash appears.
          // The functional form reads the latest count instead of the one this render closed over,
          // which matters if a click lands while an optimistic write is still in flight.
          setDrawCount(function (previous) {
            return previous + 1;
          });
        ...
        var bell = React.createElement(
          'button',
          {
            ...
            // rev-24 · arms the slash draw. 'false' until this bell has been clicked once, so a page that
            // loads already-muted shows the stroke fully drawn rather than drawing it (see BELL_MUTE_MS).
            'data-draw': drawCount > 0 ? 'true' : 'false',
            title: label,
            'aria-label': label,
            'aria-pressed': on,
            onClick: toggle,
          },
          bellIcon(on !== true, drawCount),
        );
      ...
      var diagnostics = {
        ...
        sessionIcon: {
          ...
          /** rev-24: how long ONE mute gesture takes — it drives the slash AND the blue fill, both directions. */
          bellMuteMs: BELL_MUTE_MS,
          /** rev-24: the drawn length of that diagonal, derived from its two endpoints. */
          bellSlashLen: BELL_SLASH_LEN,
