 ...
 *   global settings) and is reported, never thrown.
...
(function () {
  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
       ...
       * behavioural edit.
       */
      var REVISION = 'rev-11 · per-session chime (race fix)';
       ...
       *
       * A failed read leaves the table EMPTY (`ready` false, an error line for the
       * popover): "no overrides" means every session follows the global settings,
       * which is the honest state when the store cannot be reached. Never rejects —
       * a rejection here would surface as an unhandled rejection in the page.
       */
      function refreshSessions() {
        if (typeof fetch !== 'function') {
          sessions.error = 'this browser cannot read the per-session overrides';
          publish();
        ...
        try {
          ...
          sessions.error = describeError(error);
          publish();
          return Promise.resolve(false);
        }
        return Promise.resolve(request)
          ...
          .then(
            function (result) {
              ...
              if (table === null) {
                sessions.error = sessionAnswerError(result);
      ...
      var DICT = {
        zh: {
          ...
          sessionHint: '每个会话的覆盖存在宿主的 approval-chime/sessions.json（最多 200 个会话）；不进设置文档、不进浏览器存储。',
        },
        en: {
          nav: 'Notifications',
          title: 'Notifications',
      ...
      function injectStyles() {
        ...
        try {
          ...
          style.textContent = [
            ...
            // rule), so it is a 20px hit target with no frame of its own, and it borrows
            // the host's interactive hover token instead of inventing a palette.
            '.dacBellWrap{position:relative;display:inline-flex;align-items:center;}',
            ...
            '.dacBell{width:20px;height:20px;}',
            '.dacCaret{width:12px;height:20px;}',
            ...
            'border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.16));background:transparent;cursor:pointer;}',
            '.dacPop select option{background-color:var(--dsw-alias-bg-layer-1,#fff);',
            'color:var(--dsw-alias-label-primary,#1a1a1a);}',
      ...
      /** The caret that opens the per-session popover: 7×10, half the bell's height. */
      function caretIcon() {
        return React.createElement(
          'svg',
          { viewBox: '0 0 8 12', width: 7, height: 10, 'aria-hidden': 'true', focusable: 'false' },
          React.createElement('path', {
            d: 'M1 1.5 6 6l-5 4.5',
            fill: 'none',
            stroke: 'currentColor',
      ...
      var diagnostics = {
         ...
         * succeeded. An unread or refused table answers `{ ready: false, sessions: {} }`
         * — the same "every session follows the global settings" state the plugin acts on.
         */
        sessions: function () {
          var copy = {};
        ...
        /**
         * rev-11: how many session writes are still in flight. Zero means the table has
         * been re-read and equals the Host file (that is the convergence point).
         */
