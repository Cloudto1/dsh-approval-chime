# Independent rev-10 full regression run (task t2, verifier).
#
# Derived from run-r7.ps1. There is no run-r8.ps1 / run-r9.ps1 in this directory:
# rev-8 and rev-9 were folded into rev-10's implementation task, so the last
# shipped run script is run-r7 and this one is derived from it. The r4/r5/r6/r7
# archives under verify-independent/_raw/ are NEVER overwritten: every log this
# script writes carries the `r10-` prefix.
#
# What rev-10 changed, and what this run therefore has to re-prove:
...
#
#   powershell -ExecutionPolicy Bypass -File verify-independent/run-r10.ps1
#
# Shell: this host has Windows PowerShell 5.1 only (no `pwsh`). The older archives
# are UTF-16LE because 5.1's `>` redirection encodes that way; every r10 log is
...
$beforePath = Join-Path $raw 'r10-baseline-before.txt'
...
foreach ($suite in @('host-half', 'client-half', 'waterfall', 'custom-audio')) {
  ...
  $log = Join-Path $raw "r10-dev-$suite.txt"
...
foreach ($probe in $probes) {
  ...
  $log = Join-Path $raw "r10-ind-$name.txt"
...
Push-Location $here
$mutationLog = Join-Path $raw 'r10-ind-probe-18-mutations.txt'
...
foreach ($mode in @(
  ...
  $raceLog = Join-Path $raw ("r10-ind-probe-18-race-" + $mode.tag + ".txt")
...
foreach ($probe in $legacyProbes) {
  $name = [System.IO.Path]::GetFileNameWithoutExtension($probe)
  $log = Join-Path $raw "r10-legacy-$name.txt"
  $code = Invoke-Logged -Body { & node $probe } -LogPath $log
  $summary = (Select-String -Path $log -Pattern '^### ' -Encoding UTF8 | Select-Object -Last 1).Line
...
foreach ($probe in $reviewerProbes) {
  $log = Join-Path $raw ("r10-reviewer-" + $probe.name.Replace('.mjs', '') + ".txt")
...
if ($failed.Count -eq 0) {
  Write-Host 'every harness suite exited 0; every probe of the run-r4 … run-r10 set exited 0 except probe-13 (declared);'
