...
report.same('lib/client.js byte count is what rev-14 claims', CLIENT_BYTES, 147062);
report.same(
  'lib/client.js sha256 is what rev-14 claims',
  CLIENT_SHA256,
  '730D1C2F7F58E19471D4B77EE43A221B4FD255BC06AD2AE3752A2C4443466BD5',
);
...
report.same('the bundleRevision badge shows the rev-14 stamp', textOf(byType(tree, 'span').find((node) => node.props.className === 'dacRev')), diagnostics.revision);
report.check('the revision stamp is the rev-14 one', /rev-14/.test(diagnostics.revision), diagnostics.revision);
