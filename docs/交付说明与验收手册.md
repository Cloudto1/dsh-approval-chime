...
4. **验收信号（可观测）**：
   ...
   - 设置导航里出现独立一行 **「通知提醒」**（在「插件」之后、「Agent 预设」之前），进入后是标题「通知提醒」+ 一行说明 + 本插件的全部控件，右上角带构建戳（当前 `rev-20 · the caret turn takes 160 ms`），**且「启用提示音」是苹果式拨动开关（开启=与音量条同色的蓝底白钮、关闭=灰底）**；
...
```js
window.__DSH_APPROVAL_CHIME__.revision      // 'rev-20 · the caret turn takes 160 ms'（判断页面是否旧 bundle）
...
所以排查顺序是：先在控制台读 `.revision`（应为 `rev-20 …`）确认拿到的是本版 bundle；看不到就**先硬刷新**；
