...
>
> 一句话：**「移过去、单独开一块」在字节、DSH 语义、桩槽账本、页面控件四个层面都成立，而且是零丢失、零回归**；唯一不成立的是「docs 里旧路径无残留矛盾描述」——`docs/契约调研.md` 的**一页摘要（§J 第 4 条）和挂载步骤（§E.3 第 5 步）至今仍把「注册 `settings.plugin.item` 卡片 / 在插件页看到它」当作现行做法**，`docs/验证报告.md` 的 rev-1 人工验收清单同样如此。这是**纯文档修复**（2 处改写 + 1 处加横幅），不涉及 `lib/**`。
>
...
| DSH 契约依据（只读第一手） | `<dsh-install>\node_modules\@deepseek-ai\*` |

## 1. 我的独立性与取证方式

- **没有**依赖实施者的 harness 结论：四套 harness 我自己重跑（§9.1），并**另写**一支探针 `.scratch/reviewer-r7/probe-r7-reqcheck.mjs`（自研 vm 经典脚本加载器 + 自研 React/DOM/ctx 桩 + 自己读 DSH 文件），**89 条断言 0 失败 exit 0**，并做了 1 个内存变异证明它会咬人（§9.3）。它与 `verify/_harness.mjs`、`verify-independent/**` 零共享代码。
- **没有**只读 t2 报告：DSH 槽声明/投影/交集语义/订单数字都是我打开 DSH 文件逐行读的（§4-§6 每条都给了 `文件:行号`）；t2 的 `_raw/r7-*.txt` 只用来做「同一条结论的第二来源」。
...
| # | acceptance | 判定 | 关键证据 |
| --- | --- | --- | --- |
| 1 | 成员关系：再也不注册 `settings.plugin.item`；插件配置页不出现卡片、不留空壳 | **PASS** | 源码零字面出现（`lib/**` 全仓 grep = 0）+ 桩槽账本只记 `settings.section` + `entry.key === undefined` + DSH 交集语义 `dsh-client-ui-settings-plugins/lib/client.js:1145`（§4） |
| 2 | 新分区可达且位置正确：`order 16` 紧随「插件」(15)、先于「Agent 预设」(20)；label 是 thunk、zh 为「通知提醒」 | **PASS（结构侧）** | DSH 四条 registration 第一手读出 + DSH `sort((a,b)=>a.order-b.order)` 复算 = `[general, models, plugins, approval-chime, agent-presets]`；thunk 同实例内 zh→en→zh（§5）。**导航行的真实外观/像素未证实（§10）** |
...
**(c) DSH 语义（为什么「插件 → 插件配置」必然不再出现，也不会留空壳）**：

- 插件配置页渲染的是**两道账本的交集**：`dsh-client-ui-settings-plugins/lib/client.js:1144`（`served` = DSH served 的命名空间集合）、`:1145`（`namespaces = entries().flatMap(entry => entry.options.key !== void 0 && served.has(entry.options.key) ? […] : [])`）——**只有「注册了 `key` 且该 key 被服务」的卡片才会被派发**。
- 本插件的注册带 `id`、不带 `key`（§4b）。DSH 只按 `key` 派发，`:1145` 的 `key !== void 0` 对它恒不成立 ⇒ 结构上不可能出现。
- 「不留空壳」：该标签页只在 `namespaces.length > 0` 时才渲染卡片 `<ul>`（`:414-417`），否则渲染的是标签页**自己的**空状态文案 `<p class="…empty">{t('empty')}</p>`（`:418-421`）——那是「插件配置里一张卡片都没有」时的通用文案，不是我们那张卡片的空壳；而且 DSH 自己就注册了 4 张卡片（`:1785-1810`：shell / agent-loop / subagent-model-selection / web-search），`namespaces` 在真实组合里不会为空。DSH 自己的注释 `:1093-1097` 也明写「A served namespace no card claims renders nothing」。本插件的命名空间仍被服务（DSH `lib/index.js` 注册设置命名空间），但没有任何卡片认领它 ⇒ 既不出卡片，也不出我们那一格。
...
**(a) DSH 订单一手读出**（我自己 grep 每一个包的 `name: "settings.section"` 后面的 `id`/`order`）：
...
- 按 DSH 规则复算：**`["general","models","plugins","approval-chime","agent-presets"]`**（我的探针 [PASS] 行原文），即 15 < 16 < 20 紧随「插件」。
- **id 是自用 id**：DSH 契约明文 `dsh-cordis-client-runner/lib/client.js:3882`——「Use an id of your own: a fresh id is added beside the shipped entries, while reusing a shipped id puts you in THAT cell and replaces it」；四个 shipped id 的 occupants 清单在同文件 `:3911-3916`，`approval-chime` 不在其中。
...
- DSH 契约 `dsh-cordis-client-runner/lib/client.js:3893-3894`：「A thunk is re-read on every projection, so localized text follows the active locale without re-registering.」
...
| uuid 大小写收紧 | 客户端 `:126` `CUSTOM_ID`（无 `/i`）、`:284`/`:778` 使用；DSH `lib/index.js:93` `ID_PATTERN`（无 `/i`）、schema `:284`/`:294` 带小写 pattern | PASS（4 条） |
...
四套 harness 我自己重跑：`host-half 56/56`、`client-half 143/143`、`waterfall 20/20`、`custom-audio 74/74`，**合计 293 项，4 个 exit 0**（§9.1）。`host-half` / `waterfall` 的日志与 rev-6 归档逐行相同（只差临时目录名与 run 脚本名），说明 DSH 侧与瀑布零注册确实没动。
...
| CHANGELOG rev-7 的注册形态/订单/自用 id | `CHANGELOG.md:15-32` | 与字节及 DSH 订单一手读出吻合 ✅ |
...
- **problem**：README 把这份文件称为「本插件所有 DSH 契约的**唯一依据**」（`README.md:32`），而它有两处**现行措辞**的旧入口：
...
| 文案「不重复打架」 | ✅ 导航行/大标题 =「通知提醒」，intro（`:982`）是说明句「DSH 向你申请权限时响一次…」，两者不重复 | 探针 `the intro line is present` |
...
| 真实 `dsh web` 端到端（重启 DSH → 刷新页面 → 看到新分区 → 改设置落盘） | **未证实** | 未重启 DSH、无浏览器。可证的只是**接线**：`$DSH_HOME\profiles\web\package.json` 里 `"dsh-approval-chime": "link:D:/…/dsh-approval-chime"` + `dsh.profile.bundles` 含 `dsh-approval-chime` + `node_modules` 有同名 **Junction → 插件目录**（我只读复核）；「DSH 启动时把 bundle 读进内存 / 当前进程是否已加载 rev-7 字节」**未实测** |
| 真实文件选择对话框 | **未证实** | 需要真实浏览器 + 系统选择器；桩只能证到按钮点了 `input.click()`（`lib/client.js:1530-1533`） |
| 真实音频输出/听感 | **未证实** | 无声卡；替代证据是 WebAudio 桩记录的节点图与增益 |
| 「插件 → 插件配置」界面里肉眼看不到卡片 | **结构侧成立、界面侧未证实** | §4c 的 DSH 交集语义是结构证明；肉眼确认需要真实 GUI（未证实） |
...
2. **`lib/client.js:58` / `:1768` 的过期指针**：仍是旧引用（把 `dsh-cordis-client-runner/lib/client.js:3912` 当作「Agent 预设 = order 20」的证据）。按 captain 的「`lib/**` 冻结」边界保留不改，且已在 `docs/契约调研.md:1179-1185` 与 `CHANGELOG.md:70-75` 显式记为「已知的历史注释，正确引用见 §K.1」。它属于代码注释里的**DSH 行号引用**，不是入口路径陈述，故不影响 acceptance 4。
...
| # | acceptance | 判定 | 依据 |
| --- | --- | --- | --- |
| 1 | 不再注册 `settings.plugin.item`；插件配置页无卡片、无空壳 | **PASS** | `lib/**` 逐字节未变，§4 的源码/桩槽/DSH 交集三证仍有效 |
